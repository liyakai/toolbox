...
CASE(FunctionTraitsTest_RegularFunction) {
    ...
    using traits1 = ToolBox::function_traits<decltype(test_func)>;
    ...
    using traits2 = ToolBox::function_traits<decltype(test_func2)>;
...
CASE(FunctionTraitsTest_MemberFunction) {
    ...
    using traits1 = ToolBox::function_traits<decltype(&TestClass::member_func)>;
    ...
    using traits2 = ToolBox::function_traits<decltype(&TestClass::const_member_func)>;
...
CASE(FunctionTraitsTest_Lambda) {
    ...
    using traits1 = ToolBox::function_traits<decltype(lambda1)>;
    static_assert(std::is_same_v<traits1::return_type, float>);
    static_assert(traits1::arity == 2);
    static_assert(std::is_same_v<traits1::args_tuple, std::tuple<int, double>>);

    auto lambda2 = []() -> void {};
    using traits2 = ToolBox::function_traits<decltype(lambda2)>;
