...
public:
    ...
    void OnReceived(uint64_t conn_id, const char* data, size_t size) override
    {
        ...
        if(conn_id == echo_conn_id_)
        {
            ...
            // Print("\n\n");
            Send(NT_UDP, client_conn_id, send_data, send_data_size);
        ...
        } else 
        {
            ...
            Send(NT_UDP, echo_conn_id_, send_data, send_data_size);
...
CASE(test_udp_echo)
{
    ...
    Singleton<TestNetworkEcho>::Instance()->Accept("127.0.0.1", 9600, NT_UDP, 10*1024*1024, 10*1024*1024);
...
CASE(test_udp_forward)
{
    ...
    Singleton<TestNetworkForward>::Instance()->Accept("127.0.0.1", 9500, NT_UDP);
    Singleton<TestNetworkForward>::Instance()->Connect("127.0.0.1", 9600, NT_UDP, 10*1024*1024, 10*1024*1024);
