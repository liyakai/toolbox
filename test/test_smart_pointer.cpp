...
CASE(test_normal) {
  ...
  {
    ToolBox::SmartPtr<int32_t> sptr1(
        obj); // 调用普通构造函数,此时计数器 strong_count 为 1
    {
      ToolBox::SmartPtr<int32_t> sptr2(
          sptr1); // 调用拷贝构造函数,此时计数器 strong_counter 为 2
      {
        ToolBox::SmartPtr<int32_t> sptr3 =
            sptr1; // 调用赋值操作符,此时计数器 strong_counter 为 3
      }
    }
  }
