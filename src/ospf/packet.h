...
public:
     ...
     * Encode the packet.
     *
     * @param pkt std::vector into which the packet should be placed.
     * @return true if the encoding suceeded.
     */
    virtual bool encode(std::vector<uint8_t>& pkt) = 0;
    ...
    {
        OspfLogAssert(OspfTypes::V3 == get_version());
        return _instance_id;
    }
...
private:
    ...
    std::vector<uint8_t> _pkt;   // Raw packet
...
private:
    std::map<OspfTypes::Type, Packet*> _ospfv2;      // OSPFv2 Packet decoders
    std::map<OspfTypes::Type, Packet*> _ospfv3;      // OSPFv3 Packet decoders
};
...
public:
     ...
     */
    bool encode(std::vector<uint8_t>& pkt);

    // Network Mask.
    void set_network_mask(uint32_t network_mask)
    {
        OspfLogAssert(OspfTypes::V2 == get_version());
    ...
    void set_options(uint32_t options)
    {
        switch (get_version())
        {
            case OspfTypes::V2:
                if (options > 0xff)
                    OspfLogWarn("Attempt to set %#x in an 8 bit field",
                                options);
                _options = options & 0xff;
                break;
            case OspfTypes::V3:
                if (options  > 0xffffff)
                    OspfLogWarn("Attempt to set %#x in a 24 bit field",
                                options);
        ...
        switch (get_version())
        {
            ...
            case OspfTypes::V3:
                if ( router_dead_interval > 0xffff)
                    OspfLogWarn("Attempt to set %#x in a 16 bit field",
    ...
    std::list<OspfTypes::RouterID>& get_neighbours()
    {
        return _neighbours;
    }
...
private:
    ...
    std::list<OspfTypes::RouterID> _neighbours; // Router IDs of neighbours.
};
...
public:
    ...
    std::list<Lsa_header>& get_lsa_headers()
    {
        return _lsa_headers;
    }
...
private:
    ...
    std::list<Lsa_header> _lsa_headers;
};
...
public:
    ...
    std::list<Ls_request>& get_ls_request()
    {
        return _ls_request;
    }
    ...
    std::string str() const;
private:
    std::list<Ls_request> _ls_request;
};
...
public:
     ...
     * @return true if the encoding succeeded.
     */
    bool encode(std::vector<uint8_t>& pkt, uint16_t inftransdelay);

    std::list<Lsa::LsaRef>& get_lsas()
    {
        return _lsas;
    }
...
private:
    ...
    std::list<Lsa::LsaRef> _lsas;    // The std::list of LSAs in the packet.
};
...
/**
 * The definitive std::list of packets. All decoder lists should be primed
 * using this function.
 */
inline
void
...
public:
    ...
    {
        return _options & bit ? true : false;
    }

    void set_v6_bit(bool set)
    {
        OspfLogAssert(OspfTypes::V3 == _version);
        set_bit(set, V6_bit);
    }
    ...
    {
        OspfLogAssert(OspfTypes::V2 == _version);
        set_bit(set, EA_bit);
    }
    ...
    std::string pp_bool(bool val) const
    {
        return val ? "1" : "0";
    }
