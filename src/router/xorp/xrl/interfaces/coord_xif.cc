/*
 * obj/x86_64-unknown-linux-gnu/xrl/interfaces/coord_xif.cc
...
)
{
    ...
    return _sender->send(*x, callback(&XrlCoordV0p1Client::unmarshall_command, cb));
}
...
)
{
    ...
    return _sender->send(*x, callback(&XrlCoordV0p1Client::unmarshall_status, cb));
}
...
{
    Xrl* x = ap_xrl_pending.get();

    if (!x) {
        x = new Xrl(dst_xrl_target_name, "coord/0.1/pending");
        ap_xrl_pending.reset(x);
    }

    x->set_target(dst_xrl_target_name);


    return _sender->send(*x, callback(&XrlCoordV0p1Client::unmarshall_pending, cb));
}
