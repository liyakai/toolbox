/*
 * obj/x86_64-unknown-linux-gnu/xrl/interfaces/policy_redist6_xif.cc
 * vim:set sts=4 ts=8 ft=cpp:
 *
 * Copyright (c) 2001-2011 XORP, Inc and Others
 * See the XORP LICENSE.lgpl file for licensing, conditions, and warranties
 * on use.
 *
 * DO NOT EDIT THIS FILE - IT IS PROGRAMMATICALLY GENERATED
 *
 * Generated by 'clnt-gen'.
 */

#include "policy_redist6_xif.hh"

bool
XrlPolicyRedist6V0p1Client::send_add_route6(
	const char*	dst_xrl_target_name,
	const IPv6Net&	network,
	const bool&	unicast,
	const bool&	multicast,
	const IPv6&	nexthop,
	const uint32_t&	metric,
	const XrlAtomList&	policytags,
	const AddRoute6CB&	cb
)
{
    Xrl* x = ap_xrl_add_route6.get();

    if (!x) {
        x = new Xrl(dst_xrl_target_name, "policy_redist6/0.1/add_route6");
        x->args().add("network", network);
        x->args().add("unicast", unicast);
        x->args().add("multicast", multicast);
        x->args().add("nexthop", nexthop);
        x->args().add("metric", metric);
        x->args().add("policytags", policytags);
        ap_xrl_add_route6.reset(x);
    }

    x->set_target(dst_xrl_target_name);

    x->args().set_arg(0, network);
    x->args().set_arg(1, unicast);
    x->args().set_arg(2, multicast);
    x->args().set_arg(3, nexthop);
    x->args().set_arg(4, metric);
    x->args().set_arg(5, policytags);

    return _sender->send(*x, callback(&XrlPolicyRedist6V0p1Client::unmarshall_add_route6, cb));
}


/* Unmarshall add_route6 */
void
XrlPolicyRedist6V0p1Client::unmarshall_add_route6(
	const XrlError&	e,
	XrlArgs*	a,
	AddRoute6CB		cb
)
{
    if (e != XrlError::OKAY()) {
	cb->dispatch(e);
	return;
    } else if (a && a->size() != 0) {
	XLOG_ERROR("Wrong number of arguments (%u != %u)", XORP_UINT_CAST(a->size()), XORP_UINT_CAST(0));
	cb->dispatch(XrlError::BAD_ARGS());
	return;
    }
    cb->dispatch(e);
}

bool
XrlPolicyRedist6V0p1Client::send_delete_route6(
	const char*	dst_xrl_target_name,
	const IPv6Net&	network,
	const bool&	unicast,
	const bool&	multicast,
	const DeleteRoute6CB&	cb
)
{
    Xrl* x = ap_xrl_delete_route6.get();

    if (!x) {
        x = new Xrl(dst_xrl_target_name, "policy_redist6/0.1/delete_route6");
        x->args().add("network", network);
        x->args().add("unicast", unicast);
        x->args().add("multicast", multicast);
        ap_xrl_delete_route6.reset(x);
    }

    x->set_target(dst_xrl_target_name);

    x->args().set_arg(0, network);
    x->args().set_arg(1, unicast);
    x->args().set_arg(2, multicast);

    return _sender->send(*x, callback(&XrlPolicyRedist6V0p1Client::unmarshall_delete_route6, cb));
}


/* Unmarshall delete_route6 */
void
XrlPolicyRedist6V0p1Client::unmarshall_delete_route6(
	const XrlError&	e,
	XrlArgs*	a,
	DeleteRoute6CB		cb
)
{
    if (e != XrlError::OKAY()) {
	cb->dispatch(e);
	return;
    } else if (a && a->size() != 0) {
	XLOG_ERROR("Wrong number of arguments (%u != %u)", XORP_UINT_CAST(a->size()), XORP_UINT_CAST(0));
	cb->dispatch(XrlError::BAD_ARGS());
	return;
    }
    cb->dispatch(e);
}
